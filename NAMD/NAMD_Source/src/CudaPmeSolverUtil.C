...
//Use cufft on NVIDIA architectures.
#ifdef NAMD_CUDA
...
//On AMD, cuFFT can't be called, and HipFFT is missing alot. Directly plan through rocFFT
#else
void CudaFFTCompute::createPlans(
    rocfft_transform_type forwardTransformType, rocfft_transform_type backwardTransformType,
    size_t dimensions, const size_t* lengths, size_t transforms)
{
  rocfftCheck(
    rocfft_plan_create(
      &forwardPlan,
      ...
      dimensions, lengths, transforms, nullptr
    )
  );
  rocfftCheck(
    rocfft_plan_create(
  ...
  rocfftCheck(rocfft_execution_info_create(&forwardPlanInfo));
  rocfftCheck(rocfft_execution_info_create(&backwardPlanInfo));

  size_t workBufferSize;
  rocfftCheck(rocfft_plan_get_work_buffer_size(forwardPlan, &workBufferSize));
  if(workBufferSize > 0) {
      cudaCheck(hipMalloc(&forwardWorkBuffer, workBufferSize));
      rocfftCheck(rocfft_execution_info_set_work_buffer(forwardPlanInfo, forwardWorkBuffer, workBufferSize));
  } else {
    forwardWorkBuffer = nullptr;
  }
  rocfftCheck(rocfft_plan_get_work_buffer_size(backwardPlan, &workBufferSize));
  if(workBufferSize > 0) {
      cudaCheck(hipMalloc(&backwardWorkBuffer, workBufferSize));
      rocfftCheck(rocfft_execution_info_set_work_buffer(backwardPlanInfo, backwardWorkBuffer, workBufferSize));
  } else {
    backwardWorkBuffer = nullptr;
  }
}

void CudaFFTCompute::plan3D(int *n, int flags) {
  cudaCheck(hipSetDevice(deviceID));
  size_t lengths[3] = { (size_t)n[0], (size_t)n[1], (size_t)n[2] };
  createPlans(
...
void CudaFFTCompute::plan2D(int *n, int howmany, int flags) {
  cudaCheck(hipSetDevice(deviceID));
  size_t lengths[2] = { (size_t)n[0], (size_t)n[1] };
  createPlans(
    rocfft_transform_type_real_forward, rocfft_transform_type_real_inverse,
    2, lengths, howmany
  );
...
CudaFFTCompute::~CudaFFTCompute() {
  cudaCheck(hipSetDevice(deviceID));
  rocfftCheck(rocfft_plan_destroy(forwardPlan));
  rocfftCheck(rocfft_plan_destroy(backwardPlan));
  rocfftCheck(rocfft_execution_info_destroy(forwardPlanInfo));
  rocfftCheck(rocfft_execution_info_destroy(backwardPlanInfo));
  if (forwardWorkBuffer != nullptr)
  {
    cudaCheck(hipFree(forwardWorkBuffer));
  }
  if (backwardWorkBuffer != nullptr)
  {
    cudaCheck(hipFree(backwardWorkBuffer));
  }
  if (dataSrcAllocated) deallocate_device<float>(&dataSrc);
  if (dataDstAllocated) deallocate_device<float>(&dataDst);
}
...
void CudaFFTCompute::forward() {
  cudaCheck(hipSetDevice(deviceID));
  rocfftCheck(rocfft_execute(forwardPlan, (void**)&dataSrc, (void**)&dataDst, forwardPlanInfo));
}

void CudaFFTCompute::backward() {
  cudaCheck(hipSetDevice(deviceID));
  rocfftCheck(rocfft_execute(backwardPlan, (void**)&dataDst, (void**)&dataSrc, backwardPlanInfo));
}

void CudaFFTCompute::setStream() {
  cudaCheck(hipSetDevice(deviceID));
  rocfftCheck(rocfft_execution_info_set_stream(forwardPlanInfo, stream));
  rocfftCheck(rocfft_execution_info_set_stream(backwardPlanInfo, stream));
}
#endif
//###########################################################################
//###########################################################################
