...
#ifdef NAMD_CUDA  //Handles NVIDIA
                 ...
                 float* __restrict__ devForcesSlow_z)
{
  ...
  if (force.x != 0.0f || force.y != 0.0f || force.z != 0.0f) {
    atomicAddNoRet(&devForces_x[pos], force.x);
    atomicAddNoRet(&devForces_y[pos], force.y);
    atomicAddNoRet(&devForces_z[pos], force.z);
  }
  if (doSlow) {
    if (forceSlow.x != 0.0f || forceSlow.y != 0.0f || forceSlow.z != 0.0f) {
      atomicAddNoRet(&devForcesSlow_x[pos], forceSlow.x);
      atomicAddNoRet(&devForcesSlow_y[pos], forceSlow.y);
      atomicAddNoRet(&devForcesSlow_z[pos], forceSlow.z);
    }
  }
#else
...
__global__ void
#ifndef NAMD_CUDA
// TODO-HIP: The second parameter has different meaning on HCC.
// Using 10 as above reduces performance dramatically.
...
nonbondedForceKernel(
  ...
  const int vdwCoefTableWidth, const float2* __restrict__ vdwCoefTable, cudaTextureObject_t vdwCoefTableTex, 
  ...
  // The line above is the CUDA-exclusive version. HIPification in this case is selecting based on specific parameters
  int itileList = start + (NONBONDKERNEL_NUM_WARP == 1 ? blockIdx.x : (threadIdx.x/WARPSIZE + NONBONDKERNEL_NUM_WARP*blockIdx.x));  if (itileList < numTileLists)
  {
    ...
    // Start computation
    {
      ...
      for (int jtile=jtileStart;jtile <= jtileEnd;jtile++) {
        ...
        if (doPairlist && WARP_ANY(WARP_FULL_MASK, nexcluded & 1)) {
          int anyexcl = (65536 | WARP_ANY(WARP_FULL_MASK, excl != 0));
...
#pragma unroll
      for (int i=WARPSIZE/2;i >= 1;i/=2) {
        ...
        if (doSlow) energySlow += WARP_SHUFFLE_XOR(WARP_FULL_MASK, energySlow, i, WARPSIZE);
      }
  ...
  for (int ibase = blockIdx.x*blockDim.x;ibase < numTileLists;ibase += blockDim.x*gridDim.x)
  {
    ...
    if (doEnergy) {
      ...
      if (doSlow) {
        ...
        if (threadIdx.x == 0) atomicAdd(&virialEnergy[bin].energySlow, energySlow);
      }
      // if (doGBIS) {
...
#define CALL(DOENERGY, DOVIRIAL, DOSLOW, DOPAIRLIST, DOSTREAMING) \
...

    {
      ...
      if (doSlow) 
        transposeForcesKernel<1><<<grid, block, 0, stream>>>(
                       d_forces, d_forcesSlow,
                       force_x, force_y, force_z, force_w,
                       forceSlow_x, forceSlow_y, forceSlow_z, forceSlow_w,
                       atomStorageSize);
      else
        transposeForcesKernel<0><<<grid, block, 0, stream>>>(
                       d_forces, d_forcesSlow,
                       force_x, force_y, force_z, force_w,
                       forceSlow_x, forceSlow_y, forceSlow_z, forceSlow_w,
                       atomStorageSize);        
...
#ifdef NAMD_CUDA
// TODO-HIP: cudaMemcpyToSymbol crashes on HIP-hcc with single or multiple GPUs.
// Disable it considering that using constant memory does not improve performance.
// This explains some choices made above.
