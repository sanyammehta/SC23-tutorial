...
void getPathCpu(float* path, size_t sampleNum, float dt, monteCarloOptionStruct optionStruct, unsigned int thread_id)
{
        ...
        for (size_t i=1; i<SEQUENCE_LENGTH; i++) 
	{
            	float t = i*dt; 
		seed = 42 + i + thread_id;
		/* Changed rand() to thread-safe rand_r() for parallel performance - Sanyam Mehta */
		float randVal = ((float)rand_r(&seed)) / ((float)RAND_MAX);
...
void monteCarloGpuKernelCpuOpenMP(float* samplePrices, float* sampleWeights, float* times, float dt, monteCarloOptionStruct* optionStructs, int numSamples)
{
	dataType path[SEQUENCE_LENGTH];
  	dataType price;
	#pragma omp parallel for schedule(static) default(none) shared(samplePrices,sampleWeights,times,dt,optionStructs,numSamples) firstprivate(path) private(price)
	for (size_t numSample = 0; numSample < numSamples; numSample++)
	{
		...
		getPathCpu(path, numSample, dt, optionStructs[optionStructNum], omp_get_thread_num());
...
void monteCarloGpuKernelCpu(float* samplePrices, float* sampleWeights, float* times, float dt, monteCarloOptionStruct* optionStructs, int numSamples)
{
	for (size_t numSample = 0; numSample < numSamples; numSample++)
	{
		...
		getPathCpu(path, numSample, dt, optionStructs[optionStructNum],0);
		float price = getPriceCpu(path[SEQUENCE_LENGTH-1]);
