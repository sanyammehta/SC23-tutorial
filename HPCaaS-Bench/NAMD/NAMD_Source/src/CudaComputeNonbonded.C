...
void CudaComputeNonbonded::finishPatch(int i) {
  ...
  char buf[64];
  sprintf(buf, "%s: %d", NamdProfileEventStr[NamdProfileEvent::COMPUTE_NONBONDED_CUDA_FINISH_PATCHES], i);
  NAMD_EVENT_START_EX(1, NamdProfileEvent::COMPUTE_NONBONDED_CUDA_FINISH_PATCHES, buf);
#endif
  PatchRecord &pr = patches[i];
